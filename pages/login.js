...
function validatePassingYear(year) {
    ...
    return numYear >= 1970 && numYear <= 2028;
}

function showYearPicker(input) {
    // Remove any existing picker
    document.querySelectorAll('.year-picker-dropdown').forEach(e => e.remove());
    ...
    for (let y = maxYear; y >= minYear; y--) {
        ...
        yearBtn.style.padding = '8px 16px';
        yearBtn.style.cursor = 'pointer';
        yearBtn.onmouseover = () => yearBtn.style.background = '#f0f0f0';
        yearBtn.onmouseout = () => yearBtn.style.background = '';
        yearBtn.onclick = () => {
            input.value = y;
            picker.remove();
        };
        picker.appendChild(yearBtn);
    }
...
const API_BASE_URL = 'https://lndalumni.in';
